...
must_haves:
  truths:
    ...
    - "No commented-out code remains in transform modules"
...
6. Verify no commented-out code remains:
   `grep -r "// ctx\.\|// self\.\|// let " optimizer/src/transform/`
   Should be empty (no commented-out code lines).

7. Verify docs generate cleanly:
...
`wc -l optimizer/src/transform/*.rs optimizer/src/transform/jsx/*.rs | tail -1` - shows reduced line count
  ...
  <done>All 239 tests pass, line reduction verified (>= 600 lines removed from 3701), no commented-out code remains.</done>
</task>
...
4. No commented-out code remains
5. Documentation generates cleanly
...
- Requirements RED-01 through RED-05 satisfied:
  ...
  - RED-03: SWC parity comments removed (only comments exist, no code)
