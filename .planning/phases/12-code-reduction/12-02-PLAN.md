...
must_haves:
  truths:
    ...
    - "No SWC parity comments remain (these are comments only - no SWC parity code exists)"
...
Note on RED-03 (SWC parity code removal): Per RESEARCH.md analysis (lines 263-274), only SWC parity COMMENTS exist in the codebase (17 occurrences like `// (SWC fold_call_expr)`). No actual SWC parity CODE blocks exist that need removal. This plan removes those SWC parity comments, which fully satisfies RED-03.
</objective>
...
1. generator.rs (~150 comment lines):
   ...
   - Remove all SWC parity comments (these are the ONLY SWC parity content - no code blocks):
...
Run: `cargo check --package qwik-optimizer` - compiles without errors.
  ...
  <done>Core modules (generator, options, state, mod) have inline comments removed, including all SWC parity comments.</done>
</task>
...
- All SWC parity comments removed (satisfies RED-03 - only comments exist, no code blocks)
- Public API doc comments preserved
- All 239 tests pass
- ~300 lines reduced
</success_criteria>
