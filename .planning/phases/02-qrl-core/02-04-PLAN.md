...
must_haves:
  ...
  key_links:
    - from: "optimizer/src/code_move.rs"
      ...
      via: "uses scoped_idents from SegmentData"
      pattern: "scoped_idents"
---
...
<task type="auto">
  ...
  <action>
Update QrlComponent to use code_move for segment code generation:

1. Add method to generate segment module code:
```rust
impl QrlComponent {
    /// Generates the code for this segment's module file.
    ///
    /// The generated module includes:
    /// - Import for useLexicalScope (if scoped_idents is non-empty)
    /// - Imports for all local_idents
    /// - The transformed function expression as a named export
    pub fn generate_segment_code<'a>(
        &self,
        ast: &AstBuilder<'a>,
        ...
        let transformed_expr = if self.segment_data.has_captures() {
            transform_function_expr(
                self.expr.clone(),  // Need to store expr in QrlComponent
                "useLexicalScope",
        ...
        // Generate module with imports + named export
        // ... build Program AST and codegen
    }
}
```

2. Store the original expression in QrlComponent (may already be stored or need adding):
```rust
pub struct QrlComponent {
    // ... existing fields
    pub expr: Option<Expression<'static>>,  // The extracted function
}
```

3. Update `from_call_expression_argument` to store the expression.

Note: The exact implementation depends on how QrlComponent currently handles the extracted expression. The key requirement is that when generating segment file code, we apply transform_function_expr if there are captures.
  </action>
  <verify>
Run `cargo check -p optimizer` - compiles.
...
transform_function_expr is called when segment_data.has_captures().
  </done>
</task>
...
4. 5 unit tests pass
5. QrlComponent uses transform for segment code generation
</success_criteria>
