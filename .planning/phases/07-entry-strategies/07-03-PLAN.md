...
4. Find where QrlComponent is created (in exit_call_expression or similar).
When creating a QrlComponent, call entry_policy to get the entry value:
```rust
let entry = self.entry_policy.get_entry_for_sym(&self.stack_ctxt, &segment_data);
```

5. Add entry field to QrlComponent struct if not present:
- Check `optimizer/src/component/component.rs`
- If QrlComponent doesn't have `entry: Option<String>`, add it
- Pass the entry value when creating QrlComponent

6. Update callers of TransformGenerator::new():
- In transform.rs: optimize_module() or similar entry point
...
Parse it with parse_entry_strategy() to get the Box<dyn EntryPolicy>.
  ...
  <done>entry_policy field added to TransformGenerator and called during segment creation</done>
</task>
...
4. Ensure QrlComponent struct has the entry field:
- Check optimizer/src/component/component.rs
- If entry field doesn't exist, add it: `pub entry: Option<String>`
- Update QrlComponent creation to include entry value
...
4. SmartStrategy: stateless handlers entry = None, others grouped
  ...
  <verify>cargo test --package qwik-optimizer entry_strategy 2>&1 | tail -50</verify>
  <done>Integration tests verify all entry strategies produce correct segment grouping</done>
</task>
