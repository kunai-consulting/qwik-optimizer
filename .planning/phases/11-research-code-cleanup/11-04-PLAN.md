...
<task type="auto">
  ...
  <files>optimizer/src/transform/qrl.rs, optimizer/src/transform/mod.rs, optimizer/src/transform/generator.rs</files>
  <action>
1. Verify TransformGenerator field visibility for module access:
   - Check that TransformGenerator fields needed by qrl.rs have `pub(crate)` visibility
   - Fields that qrl.rs will need access to include: segment_stack, qrl_ctx, and any QRL-related state
   - If fields are private, update them to `pub(crate)` in generator.rs

2. Create optimizer/src/transform/qrl.rs

3. Extract QRL-related logic from generator.rs:
...
4. Create helper functions for call_expression handling (QRL extraction):
   ```rust
...
Verify field visibility: `grep -n "pub(crate)" optimizer/src/transform/generator.rs` should show QRL-related fields.
  </verify>
  <done>
- qrl.rs exists with QRL extraction logic
- compute_scoped_idents is in qrl.rs
- TransformGenerator fields have pub(crate) visibility for module access
- Module compiles
  </done>
</task>
...
<task type="auto">
  ...
  <files>optimizer/src/transform/scope.rs, optimizer/src/transform/mod.rs, optimizer/src/transform/generator.rs</files>
  <action>
1. Verify TransformGenerator field visibility for module access:
   - Check that TransformGenerator fields needed by scope.rs have `pub(crate)` visibility
   - Fields that scope.rs will need access to include: decl_stack, and any scope-tracking state
   - If fields are private (and not already updated in Task 1), update them to `pub(crate)` in generator.rs

2. Create optimizer/src/transform/scope.rs

3. Extract scope tracking logic from generator.rs:
...
6. Update optimizer/src/transform/mod.rs:
   - Add `mod scope;`
  </action>
  <verify>
Run `cargo check -p optimizer` - should compile.
Verify field visibility: `grep -n "pub(crate)" optimizer/src/transform/generator.rs` should show scope-related fields.
  </verify>
  <done>
