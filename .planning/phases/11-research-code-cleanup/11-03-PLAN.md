...
1. Verify TransformGenerator field visibility for module access:
   - Check that TransformGenerator fields (jsx_stack, jsx_mutable_props_obj, ctx, etc.) have `pub(crate)` visibility
   - If fields are private, update them to `pub(crate)` in generator.rs so jsx.rs can access them
   - Fields that jsx.rs will need access to include: jsx_stack, jsx_mutable_props_obj, and any other JSX-related state

2. Create optimizer/src/transform/jsx.rs

3. Extract these functions from generator.rs to jsx.rs:
...
4. Create public helper functions that will be called from Traverse impl:
   ```rust
...
5. Similarly create helpers for:
...
Verify field visibility: `grep -n "pub(crate)" optimizer/src/transform/generator.rs` should show JSX-related fields.
  </verify>
  <done>
