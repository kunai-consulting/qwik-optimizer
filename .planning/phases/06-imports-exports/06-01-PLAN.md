...
must_haves:
  ...
  key_links:
    ...
    - from: "optimizer/src/transform.rs export_by_name"
      to: "Plan 03 segment import generation"
      via: "export_by_name lookup for referenced_exports"
...
**NOTE:** The `export_by_name` HashMap created here will be consumed by Plan 03 (segment import generation). Plan 03's Task 2 will use `self.export_by_name.get(name)` to determine which identifiers in QRL bodies are source exports that need to be imported in segment files.
  </action>
